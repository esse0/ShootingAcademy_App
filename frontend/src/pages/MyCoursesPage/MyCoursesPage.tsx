...
function MyCoursesPage() {
    ...
    const { resData: RecivedMyCourses, execute } = useApi<MyCourseBannerType[]>(
        async () => {
            return axios.get('/api/course/user', {
    ...
    useEffect(() => {
        ...
        if (searchText) {
            setFilteredMyCourses(
                RecivedMyCourses.filter((mucourse) =>
                    mucourse.title
                        .toLowerCase()
                        .includes(searchText.toLowerCase()),
                ).filter((course) => course.is_closed == false),
            );
        } else {
            setFilteredMyCourses(
                RecivedMyCourses.filter((course) => course.is_closed == false),
            );
        }
