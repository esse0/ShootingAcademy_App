...
function MyCompetitionsPage() {
  ...
  const [filteredMyCompetitions, setFilteredMyCompetitions] = useState<CompetitionType[]>([]);

  const {resData: RecivedMyCompetitions, execute} = useApi<CompetitionType[]>(async ()=>{
    return axios.get('/api/competition/user', {params: {history: false}});
  })
    ...
    useEffect(() => {
        ...
        if(searchText){
          setFilteredMyCompetitions(RecivedMyCompetitions.filter(RecivedMyCompetitions => RecivedMyCompetitions.title.toLowerCase().includes(searchText.toLowerCase())))
        }else{
     ...
     return (
       <>
          <div style={{position: 'relative', display: 'inline-block', margin: "0 0 10px 0", width: '330px'}}>
            <SearchIcon style={{position: 'absolute', left: 10, top: 10, width: 20, height: 20}}/>
            <TextField
                  fullWidth
                  inputMode="search"
                  placeholder="Search"
